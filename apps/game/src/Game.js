...
export class Game {
  constructor(canvasId, options = {}) {
    ...
    // Set up ship callbacks
    this.ship.onAction = (actionType, actionData) => {
      if (
        this.multiplayer &&
        this.networkClient &&
        this.networkClient.isConnected()
      ) {
        // Send state update instead of action
        this.sendShipStateUpdate();
      }
    };

    this.ship.onEnergyChange = (energy, shieldPower) => {
      if (
        this.multiplayer &&
        this.networkClient &&
        this.networkClient.isConnected()
      ) {
        // Send state update with energy change
        this.sendShipStateUpdate();
      }
    };
    ...
    this.input.onLaunch((angle, power) => {
      ...
      if (this.ship.energy >= launchCost) {
        ...
        // In multiplayer, send state update to server
        if (
          this.multiplayer &&
          this.networkClient &&
          this.networkClient.isConnected()
        ...
        this.ship.launch(angle, power, this.physics);
      } else {
  ...
  initNetworkClient(serverUrl, playerName) {
    ...
    this.networkClient.onPlayerStateUpdate = (playerId, state) => {
      ...
      if (networkShip) {
        networkShip.updateState(state);
        console.log(
          `[CLIENT] Updated network player ${playerId.substring(0, 8)} state`
        );
      }
    };

    this.networkClient.onAction = (action) => {
      // Legacy support - convert action to state update
      console.log(
        `[CLIENT] Received action: ${action.type} from player ${
          action.playerId?.substring(0, 8) || "unknown"
        }`
      );
    ...
    this.networkClient.onPlayerJoined = (playerId, playerName, playerCount) => {
      console.log(
        `[CLIENT] Player joined: ${playerName} (${playerId.substring(
          0,
          8
    ...
    this.networkClient.onPlayerLeft = (playerId, playerCount) => {
      console.log(
        `[CLIENT] Player left: ${playerId.substring(
          0,
          8
        )}, total players: ${playerCount}`
      );
  ...
  handleStateSnapshot(snapshot, timestamp) {
    ...
    console.log(
      `[CLIENT] Received state snapshot: ${
    ...
    if (snapshot.width && snapshot.height) {
      if (this.width !== snapshot.width || this.height !== snapshot.height) {
        console.log(
          `[CLIENT] World dimensions changed: ${this.width}x${this.height} -> ${snapshot.width}x${snapshot.height}`
        );
    ...
    if (this.stars.length === 0 && snapshot.stars) {
      console.log(
        `[CLIENT] Syncing ${snapshot.stars.length} stars from server`
      );
    ...
    if (this.obstacles.length !== obstaclesBefore) {
      console.log(
        `[CLIENT] Obstacles synced: ${obstaclesBefore} -> ${this.obstacles.length}`
      );
    }
    ...
    if (this.networkPlayers.size !== playersBefore) {
      console.log(
        `[CLIENT] Network players synced: ${playersBefore} -> ${this.networkPlayers.size}`
      );
    }
  }
  ...
  handleAction(action) {
    ...
    console.log(
      `[CLIENT] Received action: ${type} from player ${
        playerId?.substring(0, 8) || "unknown"
      }`
    );
  ...
  handlePlayerFire(action) {
    ...
    if (!networkShip) {
      console.warn(
        `[CLIENT] Cannot handle fire action: network ship ${action.playerId?.substring(
          0,
          8
        )} not found`
      );
    ...
    // Use ship's fire method
    const laser = networkShip.fire();
    if (laser) {
      this.lasers.push(laser);
      console.log(
        `[CLIENT] Applied fire action: created laser from player ${action.playerId?.substring(
          0,
          8
        )}`
      );
  ...
  handlePlayerLaunch(action) {
    ...
    if (!networkShip) {
      console.warn(
        `[CLIENT] Cannot handle launch action: network ship ${action.playerId?.substring(
          0,
          8
        )} not found`
      );
    ...
    // Use ship's launch method
    networkShip.launch(
      action.engineAngle,
      action.enginePower / 10,
      this.physics
    );
    console.log(
      `[CLIENT] Applied launch action: player ${action.playerId?.substring(
        0,
        8
      )}`
    );
  ...
  handlePlayerShieldChange(action) {
    ...
    if (!networkShip) {
      console.warn(
        `[CLIENT] Cannot handle shield change action: network ship ${action.playerId?.substring(
          0,
          8
        )} not found`
      );
    ...
    // Use ship's setShieldState method
    networkShip.setShieldState(
      action.shieldEnergized,
      action.shieldAngle,
      action.shieldPower
    );
    console.log(
      `[CLIENT] Applied shield change action: player ${action.playerId?.substring(
        0,
        8
      )} shield energized: ${action.shieldEnergized}`
    );
  }
  ...
  handlePlayerEnergyChange(action) {
    ...
    if (action.playerId === myPlayerId) {
      // Use ship's updateState method
      this.ship.updateState({
        energy: action.energy,
        shieldPower: action.shieldPower,
      });
      console.log(
        `[CLIENT] Applied energy change to main ship: energy ${action.energy.toFixed(
          1
        )}`
      );
      ...
      if (this.energyBar) {
        this.energyBar.style.width = `${
          (this.ship.energy / this.ship.maxEnergy) * 100
        }%`;
        this.energyBar.style.backgroundColor =
          this.ship.energy < 20 ? "#ff0000" : "#0f0";
      }
      return;
    }
    ...
    if (!networkShip) {
      ...
      if (npcShip) {
        ...
        console.log(
          `[CLIENT] Applied energy change to NPC ${action.playerId?.substring(
            0,
            8
          )}`
        );
      ...
      console.warn(
        `[CLIENT] Cannot handle energy change action: ship ${action.playerId?.substring(
          0,
          8
        )} not found`
      );
    ...
    // Use ship's updateState method
    networkShip.updateState({
      energy: action.energy,
      shieldPower: action.shieldPower,
    });
    console.log(
      `[CLIENT] Applied energy change to network player ${action.playerId?.substring(
        0,
        8
      )}`
    );
    ...
    serverObstacles.forEach((serverObs) => {
      ...
      if (existing) {
        existing.updateState({
          position: { x: serverObs.x, y: serverObs.y },
          size: serverObs.size,
        });
        ...
        obstacle.onDestroyed = (laserId) => {
          const isMyLaser = laserId === this.ship.id;
          if (isMyLaser) {
            this.ship.score += 50;
            this.updateScoreDisplay();
          }
  ...
  syncNetworkPlayers(serverPlayers) {
    ...
    serverPlayers.forEach((playerState) => {
      ...
      if (this.networkPlayers.has(playerState.id)) {
        ...
        if (dx > 1 || dy > 1) {
          console.log(
            `[CLIENT] Updated network player ${playerState.id.substring(
              0,
              8
            )}: pos (${oldPos.x.toFixed(1)}, ${oldPos.y.toFixed(
              1
            )}) -> (${playerState.position.x.toFixed(
              1
            )}, ${playerState.position.y.toFixed(
              1
            )}), energy: ${playerState.energy.toFixed(1)}`
          );
        }
      } else {
        // Create new network player
        console.log(
          `[CLIENT] Creating new network player ${playerState.id.substring(
            0,
            8
          )} at (${playerState.position.x.toFixed(
            1
    ...
    for (const [playerId, networkShip] of this.networkPlayers.entries()) {
      if (!serverPlayers.find((s) => s.id === playerId)) {
        console.log(
          `[CLIENT] Removing network player ${playerId.substring(0, 8)}`
        );
        const index = this.npcShips.indexOf(networkShip);
        if (index > -1) {
          this.npcShips.splice(index, 1);
  ...
  hasStateChanged(position, velocity, energy) {
    const posChanged =
      Math.abs(position.x - this.lastSentState.position.x) >
        this.stateChangeThreshold.position ||
      Math.abs(position.y - this.lastSentState.position.y) >
        this.stateChangeThreshold.position;

    const velChanged =
      Math.abs(velocity.x - this.lastSentState.velocity.x) >
        this.stateChangeThreshold.velocity ||
      Math.abs(velocity.y - this.lastSentState.velocity.y) >
        this.stateChangeThreshold.velocity;

    const energyChanged =
      Math.abs(energy - this.lastSentState.energy) >
      this.stateChangeThreshold.energy;

    return posChanged || velChanged || energyChanged;
  }

  /**
   * Send ship state update to server
   */
  sendShipStateUpdate() {
    if (
      !this.multiplayer ||
    ...
    // Send full ship state update
    this.networkClient.sendStateUpdate({
  ...
  /**
   * Broadcast energy change to server when laser hits (deprecated - use sendShipStateUpdate)
   */
  broadcastEnergyChange(playerId, energy, shieldPower) {
    // Just send state update instead
    this.sendShipStateUpdate();
  }
  ...
  generateObstacles(
    numObstacles = Math.max(3, Math.floor(Math.random() * 12))
  ) {
    for (let i = 0; i < numObstacles; i++) {
      ...
      if (Math.hypot(x - this.width / 2, y - this.height / 2) > 100) {
        const obstacle = new Obstacle(x, y, 20 + Math.random() * 20);
        obstacle.onDestroyed = (laserId) => {
          const isMyLaser = laserId === this.ship.id;
          if (isMyLaser) {
            this.ship.score += 50;
  ...
  update(dt) {
      ...
      if (npcShip.exploded) {
        ...
        if (isNetworkPlayer) {
          ...
          if (npcShip.energy < npcShip.maxEnergy) {
            ...
            if (npcShip.energy > npcShip.maxEnergy)
              npcShip.energy = npcShip.maxEnergy;
          }
          ...
          if (npcShip.position.x < 0 || npcShip.position.x > this.width) {
            ...
            npcShip.position.x = Math.max(
              0,
              Math.min(npcShip.position.x, this.width)
            );
          }
          if (npcShip.position.y < 0 || npcShip.position.y > this.height) {
            npcShip.velocity.y *= -0.5;
            npcShip.position.y = Math.max(
              0,
              Math.min(npcShip.position.y, this.height)
            );
          }
        }
      }
    ...
    if (!this.ship.exploded) {
      ...
      if (
        ...
        if (stateChanged) {
          console.log(
            `[CLIENT] Sending state update: pos (${this.ship.position.x.toFixed(
              1
            )}, ${this.ship.position.y.toFixed(
              1
            )}), energy ${this.ship.energy.toFixed(1)}`
          );
    ...
    for (let i = this.lasers.length - 1; i >= 0; i--) {
      ...
      if (!laser.update(dt, { width: this.width, height: this.height })) {
    ...
    this.obstacles.forEach((obs) => {
      ...
      if (dist < minDist) {
        ...
        if (this.ship.shield.active) {
          ...
          const angleToObs = Math.atan2(
            obs.position.y - this.ship.position.y,
            obs.position.x - this.ship.position.x
          );
      ...
      this.obstacles.forEach((obs) => {
        ...
        if (dist < minDist) {
          ...
          if (npcShip.shield.active) {
            const angleToObs = Math.atan2(
              obs.position.y - npcShip.position.y,
              obs.position.x - npcShip.position.x
            );
    ...
    for (let i = this.lasers.length - 1; i >= 0; i--) {
      ...
      for (let j = this.obstacles.length - 1; j >= 0; j--) {
        ...
        if (laser.checkCollision(obs, obs.size)) {
          // Use obstacle's takeDamage method
          ...
          if (isMyLaser) {
            this.ship.score += 10;
            this.ship.addEnergy(30);
            this.updateScoreDisplay();
          } else {
            this.npcShips.find((npc) => npc.id === laser.id)?.addEnergy(30);
          }

          if (destroyed || obs.isDestroyed()) {
      ...
      if (laser.id !== this.ship.id) {
        // Check player ship
        if (laser.checkCollision(this.ship, this.ship.size)) {
          this.ship.takeDamage(laser.damage, laser.position, isMyLaser);
          this.lasers.splice(i, 1);
          laserHit = true;
        }
      }
      ...
      if (!laserHit) {
        for (let k = this.npcShips.length - 1; k >= 0; k--) {
          ...
          if (laser.checkCollision(npcShip, npcShip.size)) {
            npcShip.takeDamage(laser.damage, laser.position, false);
    ...
    if (this.ship.position.x < 0 || this.ship.position.x > this.width) {
      ...
      this.ship.position.x = Math.max(
        0,
        Math.min(this.ship.position.x, this.width)
      );
    }
    if (this.ship.position.y < 0 || this.ship.position.y > this.height) {
      this.ship.velocity.y *= -0.5;
      this.ship.position.y = Math.max(
        0,
        Math.min(this.ship.position.y, this.height)
      );
    }
  }
